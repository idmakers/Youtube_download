...
def Download(link):
    ...
    #print(video_title+"is downloading\n")
    #time.sleep(0.01)
    #lock.acquire()
    #no = len
    #print(str(no)+video_title+"\n")
    #lock.release()
...
def onComplete(stream, file_path):
    ...
    if download_count == 1:
        ...
        # 合併聲音檔
        merge_media()
if input("contunue  q to quit\n")!= "q":        
    downloadbypl= input("Do you want download by platlist? y for yes n for no \n")
    if downloadbypl == "y":
